...
func Fork(githubHandle string, token tokenProvider, upstreamLocation string, toLocation string, withBranchName string) (Repo, error) {
	...
	// call fork API
	forkUpstream, err := newCreateForkFunc(packageHttpClient, token, UpstreamEnhancementsTrackingRepoApiUrl)
	if err != nil {
	...
	if err != nil {
		log.Errorf("forking upstream %s: %s", upstreamLocation, err)
		return nil, err
	}
	...
	_, err = gitRepo.CreateRemote(&config.RemoteConfig{Name: UpstreamRemoteName, URLs: []string{upstreamLocation}})
	if err != nil {
